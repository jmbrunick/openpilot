...
Reads the ROAD camera + GNSS and appends JSONL under /data. Does not write
sqlite, does not change vCruise / HUD MAX, and does not talk to osm.org.
...
def process_observations(
  ...
  if y is None or not gps_ok:
    return []
  written = []
  for sign in detector.detect(y):
...
def main():
  ...
  while True:
    ...
    if y is not None:
      process_observations(y, lat, lon, bearing, gps_ok, detector, logger, time.time())
    rk.keep_time()
