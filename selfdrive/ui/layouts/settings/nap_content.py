...
SPEED_SIGN_LOG_DESCRIPTION = (
  ...
  "process timeouts. Heavy detect never runs while openpilot is engaged "
  "(plate shows WAIT) so modeld is not starved. While you are driving "
  "manually — moving is OK — detect runs at 1 Hz and nice 19 so you can "
  "log speed-limit signs. Not gated on park or Force Offroad. OSM upload "
  "is future work; this is JSONL + HUD only. If you see TAKE CONTROL or "
  "driving-model lag, turn Logger Off and use nap-release. Does not write "
  "sqlite, does not change cruise / HUD MAX, and does not query osm.org. "
  "Stock modelV2 has no speedSign head. Real roadside detection needs the "
  "compact YOLO ONNX on /data/media/0/nap/speed_sign.onnx. If that file is "
  "missing, onroad SIGN shows NO WT — use Install weights (Wi-Fi, offroad / "
  "Force Offroad), or: python -m scripts.nap.install_speed_sign_weights."
)
...
WIPER_SPEED_DESCRIPTION = (
  ...
  "same nibble 1 only when the car is on, gear is Drive or Reverse, and "
  "the 3X road camera sees a rainy or icy/frosted windshield, then "
  "releases the real stalk when the glass looks clear or you leave Drive/"
  "Reverse. Park and Neutral never Auto-wipe. Default Off — Auto is "
  "opt-in, not every drive. DAS wiper fields stay 0 (they were ignored "
  "and caused a controls mismatch). Do not engage NAP and do not pull "
  "the stalk. Pre-AP may still see the real stalk rest on bus 0; holding "
  "nibble 1 is what already works. No spray. No auto headlights."
)
