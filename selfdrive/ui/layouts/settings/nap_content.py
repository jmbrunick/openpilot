...
DRIVER_LAT_HANDOFF_DESCRIPTION = (
  "Default On. Yields NAP lateral only on an intentional wheel turn "
  + "(sustained torsion + matching steer rate + hands on the rim) — "
  + "typically avoiding something. Long stays on. Stays yielded while "
  + "hands are on the rim. Hands off ~80 ms, then the wheel blends back "
  + "over 1 s. Gravel / wind / road-crown pressure should not trip. A "
  + "hard brake during that yield fully cancels openpilot (chime); a "
  + "light brake is still the silent long pause + one SET. Gray chrome "
  + "means lat yielded — turn Off if rumble still false-yields. "
  + "Off = stock lat, no yield."
)
