...
========================
* **Tip-brake soft-glide (not the reverted R1 regen ramp):** Pre-AP silent long pause now classifies digital Applied like the stalk. A **short tip** (Applied < **0.30 s**, then lift, not firm `aEgo`) that knocks software long off keeps interceptor ENABLE and commands a **speed-target glide** from current speed toward **~12 mph** (mid of 10–15) over **~2.5 s** (tunable 2–3). Constant decel is `clip((v_target − v0) / T, REGEN_MAX, 0)` through existing VDAS — a smooth speed ramp, not a coast plateau then stock bite, and **not** the reverted 0.75 s 0→`REGEN_MAX` (−1.5) fade. If high speed cannot reach 10–15 mph in 2–3 s within interceptor regen, command `REGEN_MAX` for that window then hand off — do not invent friction. After ego reaches ~12 mph **or** the window expires: **RELEASE** to stock (interceptor cannot friction-brake; at low speed the stock handoff is mild). Already in the 10–15 mph band: RELEASE immediately (nothing to glide). **Held** Applied ≥ **0.30 s**, **hard** (`aEgo <= −1.5`), or a second press during the glide: **RELEASE immediately** so Tesla friction + stock regen stay firm. Full cancel / FCW / AEB / hard lead while long is still on unchanged. **Kept:** #152 A+B and #153 A3 gas-lift climb. No `GAS_COMMAND` rewrite, no PostEngageCoast / climb latch / pedal-hold, no locationd or safety change. **nap-dev only — no nap-release twin.**
