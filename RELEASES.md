...
========================
* Pre-AP lead-approach hysteresis is a bit wider so occasional **bump-pull** (regen bite → rematch → bite) at the follow gap is less chatty. Enter `v_rel` **0.55** m/s (~1.2 mph), exit **0.12** m/s (~0.27 mph); was 0.50 / 0.20. Comfort peak **0.55**, onset slew **0.05**/frame, slack gates, and immediate release stay. Still closes onto Follow Distance 1–7. MPC / FCW / danger braking unchanged. Same tune as nap-dev; Hypermile is not ported.
>>>>>>> e37dd6611 (NAP: slightly wider lead-approach hysteresis (less rematch chatter))
