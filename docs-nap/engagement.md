...
6. Gas press → `OVERRIDE_LONGITUDINAL` **when One-Pedal Long is Off** (default). Pedal command passes through with `enable=0` — driver's foot controls throttle directly, NAP tracks position for smooth resume. Long engage/disengage prompts follow `enableLongControl` for **session** edges (initial engage / full cancel), not interceptor handshake, not gas override, and not a brake / one-pedal long pause. `enableLongControl` stays true while the driver is on the pedal, so press and release are silent. On the gas falling edge, ACQUIRE expires the 0.5 s engage-grace a=0 floor and seeds VDAS from `max(last non-negative aEgo, planner climb)` (clamped to the personality MAX). Open-road planner climb is Mannerisms **Acceleration** 1–10 toward HUD MAX. Lead hard decel / FCW / should-stop (`actuators.accel < 0`) still seed 0 and pass through immediately. Brake clears the pending handoff so a later SET resume keeps the normal grace ramp. First engage without prior gas is unchanged.
   - **One-Pedal Long On** (Settings → NAP → Driving Mannerisms, default Off): if OP long is **already holding with the foot off** and the accelerator is then pressed (interceptor DI **> 1**, slightly more sensitive than stock `gasPressed` DI **> 2**; not `DI_pedalPos`), **pause** `enableLongControl` with the same silent long pause as brake (`_drop_longitudinal_keep_lateral`: lat stays, sticky MAX) and **latch** `_one_pedal_pause_latched` until a stalk SET. The rest bit (`_one_pedal_had_long_at_rest` / controller `_saw_long_without_gas`) is sticky — a soft or late `gasPressed` still latches; a missed rising edge must not leave long armed. Lift / coast / regen must **not** restore long or re-ACQUIRE (stock `gasPressedOverride` ending makes `CC.longActive` true again — the latch blocks A+B / A3 and re-drops long if it comes back without SET). Do **not** `USER_DISABLE` / `hard_cancel_session` / take-control. **Engage while gas is already pressed** (one SET or two SET) does **not** pause or latch — long stays armed; lift still ACQUIREs with A+B grace expire + aEgo / A3 climb. Same-frame SET + first gas sample is engage-with-gas, not a takeover. **Lift-through DI 1–2:** stock `gasPressed` falls (DI ≤ 2) while interceptor DI is still **> 1**. That is still the engage-while-gas lift, not a from-rest tip-in — do not extra-kick a phantom rising edge (that dropped `enableLongControl`, zeroed MAX, and left Tesla regen owning the car). Orig kick passes `interceptor_di` so pause-gas stays true across 2→1; overlay `maybe_one_pedal_overlay_kick` skips `_one_pedal_gas_falling_edge`. After a takeover pause, interceptor **RELEASEs** and stays RELEASED on lift — Tesla physical pedal / stock lift-regen is the one-pedal path (REGEN_MAX envelope, ~−1.5 m/s²). **One SET** (rolling) clears the pause latch and restores long at held MAX — including SET with the foot still on gas (arms lift-to-start A+B / A3; controller `_saw_long_without_gas` must not re-latch). At a stop, that SET arms wait-for-gas like brake pause (clears the One-Pedal latch so a later gas touch can complete). Brake pause does **not** use this latch (brake resume stays as today). Standstill wait-for-gas resume and a SET that just restored long are not paused. Brake cancel is firm/stock RELEASE (not the #157 soft glide). FCW/AEB unchanged.
...
- **Maps off / posted unknown:** MAX never auto-rebases. Never invent a posted value. GPS glitch → keep held MAX. Do not latch pause ego (`cruiseState.speed`) into held. Stalk +/- while long is active updates the MAX one SET will resume — **except** a stalk tip behind a radar lead (see Follow Distance stalk below).
...
7. **Light brake** (tap, not a panic stop) while engaged, yielded or not: silent long pause + held MAX; one SET resumes **if already moving**. At a stop, one SET must **not** roll the car; tap the accelerator, then long resumes at held MAX. No disengage chime. Sticky MAX (#72+#77 / nap-release #76) unchanged. After a **short tip** that only knocks long off, interceptor stays ENABLED and ramps regen gentle→strong over ~2.5 s (feel immediately, no frame-1 stock / full-regen bite). A **held / harder** brake RELEASEs immediately to stock Tesla regen. FCW / AEB / hard lead braking must still bite.
...
15. **One-Pedal Long On:** Mannerisms toggle Off by default. Turn On. Foot off, long holding speed: press gas from rest — long should **pause** silently (lat stays, no disengage chime, session still up). Drive on the pedal; **lift must stay paused** (Tesla stock regen via interceptor RELEASE, not OP climbing back to MAX). Press gas again: accel, still no OP long, still not a full disable. **One SET** resumes at held MAX. **Foot already on the accelerator, then one or two SET pulls:** long must arm; lift must start long (A+B / A3) — must not pause. Toggle Off: gas tap then lift should resume long and climb (A3). Brake still pauses long and still resumes as today (this latch is gas-pause only). At a stop, one SET then gas still resumes (does not pause).
...
- `opendbc_repo/opendbc/car/tesla/preap/carcontroller.py` — pedal TX, engage grace, gas-lift VDAS seed
- `selfdrive/controls/tests/test_tesla_preap_gas_lift_handoff.py` — gas-lift grace expire + Accel 1–10 climb seed
- `selfdrive/controls/tests/test_tesla_preap_one_pedal_long.py` — One-Pedal Long pause latch + RELEASE regen (gas→lift keeps enableLongControl/longActive off until SET; kick-miss still latches; engage-while-gas still ACQUIREs, including lift-through DI 1–2 / dc 13:16:14; no USER_DISABLE)
- `selfdrive/controls/tests/test_tesla_preap_brake_cancel_regen.py` — brake cancel is firm RELEASE / stock regen
...
- `selfdrive/monitoring/policy.py` — stock DM + optional Simulate Look (`NAPDmSimulateLooking`, default Off) and False Alert Ignore (`NAPDmFalseAlertIgnore`, default Off); `DM_LOOKAWAY_GATE_MPH` (2 mph) look-away pause
- `selfdrive/ui/layouts/settings/hidden_toggles.py` — triple-tap NAP popup (Force Offroad, Simulate Look, False Alert Ignore)
- `selfdrive/monitoring/test_monitoring.py` — Simulate Look full-wipe recover; FAI phone-only vs pose/eye; mutual exclusion; look-away pause below 2 mph
