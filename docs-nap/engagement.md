...
6. Gas press → `OVERRIDE_LONGITUDINAL`. Pedal command passes through with `enable=0` — driver's foot controls throttle directly, NAP tracks position for smooth resume. Long engage/disengage prompts follow `enableLongControl` for **session** edges (initial engage / full cancel), not interceptor handshake, not gas override, and not a brake long pause. `enableLongControl` stays true while the driver is on the pedal, so press and release are silent. **Post-engage climb-to-MAX (until at MAX):** after software-long engage, watch the accelerator. On the **first detectable pedal decrease** (0.05 DI, not a 0.40 deadband), take over immediately with **positive accel toward MAX** and **keep climbing** until sticky/cruise MAX — do **not** sit/coast at a=0, do **not** freeze last-pressed DI on the wire, and do **not** expire at 1 s (#144/#145 pulsed and net-deceled 38→28 from a one-frame peak-DI rewrite + ACQUIRE chatter). Pedal-layer `ENGAGE_GRACE_FRAMES` (0.5 s floor at a=0 on ACQUIRE / `vdas.reset(commanded_accel=0)`) is **expired on every climb frame** so a later ACQUIRE cannot restart the grace coast. The ACQUIRE ENABLE=1 frame is rewritten with a **VDAS climb DI** (zero-torque + climb step), never last-pressed peak. Panda still blocks `ENABLE=1` while `gas_pressed` (interceptor raw > 650); Tesla passthrough regen can still run until override ends. Planner +a that is already climbing harder passes through. Sticky/cruise MAX is a hard cap. Brake pedal, FCW / should-stop, and a radar lead already asking to slow (`|a| >= 0.55`) still apply. Engage with no gas / no lift: unchanged. Does not write MAX / sticky. See `post_engage_coast.py` / `preap_post_engage_hold.py`.
...
- `selfdrive/controls/lib/post_engage_coast.py` — first pedal drop after long engage → climb toward MAX until at MAX (no a=0 coast, no 1 s drop)
- `selfdrive/car/tesla/preap_post_engage_hold.py` — expire 0.5 s engage grace, re-seed VDAS after ACQUIRE, rewrite the grace-floored ENABLE=1 with a climb DI (not last-pressed)
