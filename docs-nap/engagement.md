...
7. Stalk cancel (real, not spoof echo) → full disengage. `AudibleAlert.disengage` / `EventName.pedalCruiseDisabled` fire on that session teardown, including when openpilot is already going disabled. Door / **Reverse (or any gear out of Drive)** / permanent steer fault / hands-on ≥ 2 (when not blinker-latched) are the same hard cancel: session down, held MAX forgotten, soft-lat reset, `preap_cc_cancel_needed` so stock CC drops, disengage prompt kept. Orig `check_can_engage` only zeroed `cruiseEnabled` / long — that left sticky MAX and did not spoof CANCEL. Panda `tesla_preap` uses `pcm_cruise_check(false)` on leaving Drive **and on the Drive rising edge** (same as steering disengage) so `cruise_engaged_prev` clears. Python's CANCEL spoof is TX-only and cannot clear that latch — a real stalk disable can, which is why an extra stalk cancel→engage used to be required after R/P→Drive. Drive SET while panda is still `!controls_allowed` now pulses false then true (cancel-then-SET in one RX), so that extra cycle is not needed and does not **Controls Mismatch**. selfdrived also one-shot-resets `mismatch_counter` on the Drive rising edge **only after that engaged R/P disable** (same cleanup as stalk disable). A blanket clear on every Drive entry is not used — that would mask a real actuator/panda mismatch. After returning to Drive, a normal double SET is a new engage (not a sticky-MAX resume). **Flash the panda** after this safety change.
