...
6. Gas press → `OVERRIDE_LONGITUDINAL`. Pedal command passes through with `enable=0` — driver's foot controls throttle directly, NAP tracks position for smooth resume. Long engage/disengage prompts follow `enableLongControl` for **session** edges (initial engage / full cancel), not interceptor handshake, not gas override, and not a brake long pause. `enableLongControl` stays true while the driver is on the pedal, so press and release are silent. **Post-engage climb-to-MAX (1.0 s from the lift):** after software-long engage, watch the accelerator. On the **first detectable pedal decrease** (0.05 DI, not a 0.40 deadband), take over immediately with **positive accel toward MAX** — do **not** sit/coast at a=0 and do **not** freeze last-pressed DI (floating at the last pedal). The 1 s timer starts on the lift, so holding the pedal past 1 s after engage still hands off. Pedal-layer `ENGAGE_GRACE_FRAMES` (0.5 s floor at a=0 on ACQUIRE / `vdas.reset(commanded_accel=0)`) is **expired on the handoff** so grace cannot keep the interceptor at zero-torque. Panda still blocks `ENABLE=1` while `gas_pressed` (interceptor raw > 650); Tesla passthrough regen can still run until override ends, but the first enabled frame is already a climb (grace is dead, VDAS seeded with +a). Planner +a that is already climbing harder passes through. Sticky/cruise MAX is a hard cap. Brake pedal, FCW / should-stop, and a radar lead already asking to slow (`|a| >= 0.55`) still apply. After 1 s from the lift, or if the pedal never decreases, normal OP long (climb to MAX / regen) resumes. Does not write MAX / sticky. See `post_engage_coast.py` / `preap_post_engage_hold.py`.
...
- `selfdrive/controls/lib/post_engage_coast.py` — first pedal drop after long engage → climb toward MAX (no a=0 coast)
- `selfdrive/car/tesla/preap_post_engage_hold.py` — expire 0.5 s engage grace, seed VDAS climb, rewrite the grace-floored ENABLE=1 frame
