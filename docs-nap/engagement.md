...
7. Stalk cancel (real, not spoof echo) → full disengage. Lat and long prompts fire on those falling edges, including when openpilot is already going disabled. Door / gear / permanent steer fault / hands-on ≥ 2 (when not blinker-latched) are the same hard cancel: session down, held MAX forgotten.
