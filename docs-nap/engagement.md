...
**Default On.** Settings → NAP → Soft Lateral Handoff can be turned **Off** if gravel or crosswind still false-yields (gray HUD, OP stops fighting the path). #71 at 0.5 Nm / 80 ms tripped on rumble.
...
1. OP engaged and providing lateral. Firm sustained hand push (0.85 Nm / 250 ms) → lateral authority goes to 0. `carControl.latActive` stays **true** (no `LaC.reset`, no snap in `apply_steer_angle_limits_vm`). Longitudinal and `cruiseEnabled` stay up. Stalk cancel / doors / hands-on ≥ 2 still hard-disengage.
...
6. **Road rumble / crosswind** with hands resting. Must **not** gray. If it still does, turn Soft Lateral Handoff **Off** in Settings → NAP — do not lower panda / `STEER_THRESHOLD`.
