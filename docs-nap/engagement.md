...
6. Gas press → `OVERRIDE_LONGITUDINAL`. Pedal command passes through with `enable=0` — driver's foot controls throttle directly, NAP tracks position for smooth resume. Long engage/disengage prompts follow `enableLongControl` for **session** edges (initial engage / full cancel), not interceptor handshake, not gas override, and not a brake long pause. `enableLongControl` stays true while the driver is on the pedal, so press and release are silent. **Post-engage pedal hold (1.0 s):** after software-long engage, watch the accelerator. As soon as the pedal starts decreasing, freeze the last *pressed* interceptor DI (and the last non-negative `aEgo` it was producing) for the rest of that 1 s window — do **not** coast/hold speed or zero accel to 0 (that still ramps the pedal down to zero-torque and dips). Panda blocks `ENABLE=1` while `gas_pressed`; the `GAS_COMMAND` rewrite lands on the first enabled frame after override. Sticky/cruise MAX is a hard cap — if the held pedal would accelerate above MAX, drop the hold and let normal OP long respect MAX. Brake pedal, FCW / should-stop, and a radar lead already asking to slow (`|a| >= 0.55`) still apply. After 1 s, or if the pedal never decreases, normal OP long (climb to MAX / regen) resumes. Does not write MAX / sticky. See `post_engage_coast.py` / `preap_post_engage_hold.py`.
...
- `selfdrive/controls/lib/post_engage_coast.py` — 1 s post-engage last-pressed pedal / accel hold
- `selfdrive/car/tesla/preap_post_engage_hold.py` — rewrite ENABLE=1 `GAS_COMMAND` to the frozen DI after gas override
