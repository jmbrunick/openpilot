...
- **Held turn:** LEFT or RIGHT past 0.40s from idle → do not arm ALC; latch a driver turn (flash-latch ~1s dark). Soft-lat **Off** still pauses lat (hand release resumes). Soft-lat **On** does not clear `latActive` on lamp latch; the driver may still soft-yield, and take-back is inhibited until the latch ends (then yield + normal 0.15 s confirm + 1 s blend). A driver turn does **not** drop longitudinal — `enableLongControl` stays true and lead/map braking and accel apply through the turn. Brake still uses the silent long pause; interceptor RELEASEs immediately to stock regen.
...
**Default On** (`NAPDriverLatHandoff=1`). A light purposeful push (`|torsion| >= ~0.55 Nm` for ~90 ms + `handsOnLevel >= 1`) **frees the EPS** (`latActive` false → `DAS_steeringControlType=0`), not follow-measured angle hold. Stay yielded while hands are on the rim; 1 s blend starts after ~0.15 s of hands truly off. Emergency/hard brake (`aEgo <= −3.5 m/s²` for 80 ms + digital Applied) during yield / 2 s window fully cancels OP. Light brake is still the silent long pause. A driver-turn blinker does not strip lat when soft-lat is On; after a yielded turn, resume is yield + 0.15 s confirm + 1 s blend (no dedicated blinker blend). The same re-enable inhibit ORs **v_ego < 10 mph** (`LAT_REENABLE_MIN_V_EGO_MPH`). Turn Off in Settings → NAP → Driving Mannerisms if false-yields remain — Off also restores stock blinker lat-pause. See [engagement.md](engagement.md#driver-wheel-temporary-lateral-handoff).

## Driver monitoring (simulate looking)

Stock DM timers stay **3 / 5 / 11 s**. Looking-away / distraction alerts **do not fire below 2 mph** (`DM_LOOKAWAY_GATE_MPH` in `policy.py`) — stock standstill pause plus creeping; Simulate Look / FAI stay as toggled. Triple-tap **NAP** (1.0 s window; not under Driving Mannerisms) has three hidden toggles: **Force Offroad**, **Simulate Look**, **False Alert Ignore**. Both DM toggles default **Off** on nap-release (stock until Justin enables them) and are **mutually exclusive** (both Off is allowed; stale both-On → Simulate Look On). Shared cadence: drain **past 1.0 s**, then fire uniform in **(1.0 s, 3.0 s]**. `NAPDmSimulateLooking` is the pre-FAI **full looking-path wipe** (no-face / uncertain / phone / pose / eye) so common false nags recover on cadence. `NAPDmFalseAlertIgnore` (Sim Look Off) soft-clears **phone/device** false positives only, and only when pose and eye are not alarming. Hard cancels (hands-on ≥ 2, stalk, door, reverse) are unchanged. See [engagement.md](engagement.md#driver-monitoring-simulate-looking).

## OSM map speed (MAX)

`selfdrive/mapd` looks up OpenStreetMap `maxspeed` from an offline sqlite and publishes `liveMapDataNAP`. In pre-AP **pedal** mode, `card.py` overlays that limit onto software cruise (`CS.vCruise` / HUD **MAX**): Cap never raises, Follow tracks the sign, engage seeds to the posted limit, and a Follow stalk set (above or below) holds until the posted limit changes. A missing limit that becomes posted (none → valid) rebases MAX the same way; a posted limit that drops out (valid → none) leaves MAX unchanged. A sharp curve snapshots that MAX and restores it after the bend so temporary corner slowing cannot permanently rebase sticky. HUD current speed is wheel/ESP `vEgo` (not `vEgoCluster` / MAX). When ego is above MAX the planner commands Accel-5 comfort decel (`map_track_decel`); when ego is below MAX and there is **no** radar lead it commands Accel 1–10 climb (`map_track_accel`, last-mph taper). A valid lead owns follow — map climb must not replace non-negative MPC `a` toward MAX through that lead. Hypermile **Hill Climb** (default On while Hypermile is On; IMU pitch only — **no maps-elevation lookahead**) adds grade to that climb on a real uphill under MAX when there is no lead, and a light crest ease only at or above MAX. It never raises MAX. Coming up behind a radar lead caps **positive** close-the-gap accel to that same Accel 1–10 envelope inside ~200 m so catch-up is not the cruise 1.6–0.6 punch. Non-rapid MPC −a is floored at mild ease; FCW / rapid still own danger. `LongitudinalMpc.update(radarState, v_cruise)` still takes `min(lead, cruise)`. No-pedal stock CC is display-only. US maps are a GitHub Release download; Refresh maps overlays live OSM within 100 miles. Details: [map-speed.md](map-speed.md).

Behind a radar lead, a Pre-AP stalk **tip** (first detent, 1 mph) undoes that frame’s MAX and steps City or Highway Follow Distance 1–7 when the lever returns to IDLE. Highway when MAX is 50 mph or higher (including while still accelerating up to that MAX). City when MAX is under 50. Traveled speed picks the band only when MAX is unset. A **full press** (2nd detent, 5 mph) still steps MAX +5/−5 and does not remap Follow. No lead: tip and hold both still step MAX. See [hypermile.md](hypermile.md) and [engagement.md](engagement.md).
