...
**Default On** (`NAPDriverLatHandoff=1`). Gentle 0.70 Nm / 140 ms consecutive push yields lat; 1 s blend starts on release (no quiet wait). Turn Off in Settings → NAP if gravel or wind false-yields. `latActive` stays true (no VM snap); `desired_curvature` pins to measured while yielded so resume `clip_curvature`s from the wheel onto the model. See [engagement.md](engagement.md#driver-wheel-temporary-lateral-handoff).
