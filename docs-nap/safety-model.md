...
- Disengage on door open, gear out of Drive via `pcm_cruise_check(false)` (same re-arm as steering disengage). That clears `cruise_engaged_prev`, so returning to Drive + stalk SET is a rising edge and does not `controlsMismatch`. Python reverse/door `hard_cancel_session` still spoofs CANCEL to drop stock CC and tear down the FSM. **Flash the panda** after this safety change.
