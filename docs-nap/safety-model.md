...
- Disengage on door open, gear out of Drive via `pcm_cruise_check(false)` (same re-arm as steering disengage, including the Drive rising edge). That clears `cruise_engaged_prev`. Python reverse/door `hard_cancel_session` still spoofs CANCEL to drop stock CC, but that spoof is **TX-only** — panda does not RX its own TX, so it cannot clear the latch the way a real stalk disable does. Drive SET while `!controls_allowed` pulses `pcm_cruise_check(false)` then `true`, and TX CANCEL while already disallowed also clears the latch, so R/P→Drive does not need an extra stalk cancel→engage cycle. selfdrived one-shot-resets the mismatch counters on that Drive rising edge only after an engaged R/P disable — not on every Drive entry. **Flash the panda** after this safety change.
