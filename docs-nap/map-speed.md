...
- **A rising MAX must accelerate** (Follow, no overriding lead). MPC also will not climb to a higher MAX. Planner commands `map_track_accel` at Accel 1–10 when ego is below MAX and MPC is not braking. Stalk up / Follow posted raise also writes that higher MAX onto `pedal_speed`. Hypermile **Hill Climb** (IMU pitch only; maps-elevation lookahead NOT included) may add grade to that climb so Accel 1 does not sag; it never raises MAX.
