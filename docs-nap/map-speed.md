...
- **Follow**: MAX tracks the OSM limit. A manual set **below** the current posted limit is **sticky** (held absolute, **no 10s timeout**) until the driver stalks again or the posted limit changes. The Follow 10s timer applies only to a set **above** the limit. The timer must never clear a below-limit hold and re-Follow up to `a`.
- **Engage seed:** on `pedalLongActive` rising (second pull, not lateral-only first pull) with Cap/Follow and a valid current OSM limit, HUD MAX / `CS.vCruise` **and** `pedal_speed_kph` initialize to that limit (+ offset) so the car tracks it immediately. No valid limit → existing ego capture.
