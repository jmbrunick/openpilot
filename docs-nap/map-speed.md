...
| Late | 1.30 m/s² | 80 m | 350 m |
| Normal (default) | 1.10 m/s² | 200 m | 550 m |
| Early | 0.75 m/s² | 240 m | 600 m |

When the upcoming drop is inside that window, MAX interpolates from the current limit at the start distance to the new limit at the sign (`v²` lerp). A 50→30 mph drop at Normal starts falling ~345 m out.

**HUD current speed** is `vEgoCluster` / `vEgo` (mph/kph). OSM `maxspeed` is the LIMIT sign and, in Cap/Follow, may overlay **MAX**. Those are not the same number — a posted 50 while doing ~45 is LIMIT/MAX, not faked ego speed.
...
|---|---|---|---|
| 1 | 0.45 | **0.495 m/s²** | climb only (gentlest) |
| **5** | 1.00 | **1.10 m/s²** | climb *and* all map braking |
...
1. Cap, **no lead**: set MAX above the posted limit; the car must decelerate toward that MAX (Accel 5 ≈ 1.10 m/s²). Raising the stalk cannot exceed the cap.
