...
- **Roundabout (first slice):** detect OSM `junction=roundabout` / a compact closed circulating way ~80–100 m out — not a big steer, and not a sharp town corner or signalized cross. Soft comfort decel toward **15–20 mph** (OSM ring maxspeed when tagged, often 20). While circulating, ~0.45 m outer in-lane path bias (right in RHT / US). A short RB way is **not** dropped as a min-zone stub. Yield-before-merge, continue-circulate desire, and a UI chip are later tips.
