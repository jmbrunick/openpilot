...
Simulate Look-at-Road (`NAPDmSimulateLooking`) defaults **On**. Reveal it with Force Offroad via a **triple-tap on Settings → NAP** (3 taps within 1.0 s). While engaged, after drain is past **1.0 s**, fire a held glance at a random time in the next **2.0 s** (**(1.0 s, 3.0 s]** of that countdown) until awareness recovers to 1.0. Same rule after each full reset. Not a mute. Turn **Off** for stock DM.
