...
One-Pedal Long (`NAPOnePedalLong`) defaults **Off**. Pedal mode: gas from rest while long is **already on** **pauses** software long (silent long pause, lat stays, not a full disable) and **holds that pause until a stalk SET** — lift must not resume. SET with the foot already on the pedal still arms long; lift starts A+B/A3. Brake pause resume stays as today. Off = stock gas override + A+B/A3. Do not merge to nap-release until Justin signs off.
