...
Hands-On Look-at-Road Reset (`NAPDmHandsOnReset`) defaults **On**. Pre-AP light rim contact (`handsOnLevel >= 1`) resets only the first vision prompt. That first prompt is drawn at random in **2.0–4.5 s** each cycle (not a fixed 3 s). Orange / red still escalate. Always-on DM is unchanged. Turn **Off** for stock DM.
