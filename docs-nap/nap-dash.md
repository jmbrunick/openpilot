...
In-tree hotspot web UI on **nap-dev**. Process: `nap_dash` (`selfdrive.nap_dash.server`). Starts with openpilot when `NAPDashEnabled` is On (default).

**Do not merge** this onto `nap-release` until Justin signs off. Drive and test it from `nap-dev`.
...
- Driving Mannerisms + Map Speed + stock personality / Experimental Mode + SL / FAI (`/api/*` Params)
...
Adapted from Philip's NAP-Dash `server_v21` **UI shell**. The settings bridge is Justin's Params / Driving Mannerisms. Hypermile is not on this page.
...
```
https://installer.comma.ai/jmbrunick/openpilot/nap-dev
```

Use **`nap-dev`**. Wait for the update to finish, then reboot so manager starts `nap_dash`.
