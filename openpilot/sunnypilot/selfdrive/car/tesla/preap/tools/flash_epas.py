...
Uses ELM327 diagnostic mode only. ALLOUTPUT and panda TX bypass are rejected.
Bootloader size/SHA-256/MD5 are verified before panda connection.
"""
...
def patch_firmware(fw, offset, restore=False):
  ...
  mods: Final[tuple[tuple[int, bytes, bytes], ...]] = (
...
def flash_bootloader(uds_client, bootloader_path, start_addr):
  ...
  fw = verify_bootloader(bootloader_path)
  fw_len = len(fw)
...
def parse_args(cli_args=None):
  ...
  parser.add_argument("--accept-risk", "--confirm", dest="accept_risk", action="store_true",
  ...
  parser.add_argument("--bootloader", type=str, default=DEFAULT_BOOTLOADER_FILE,
                      help="Bootloader filename/path (relative paths are resolved under firmware/)")
...
def main(cli_args=None):
  ...
  confirmed = bool(args.accept_risk) or _consume_ui_risk_ack()
  if args.extract_only:
    from openpilot.sunnypilot.selfdrive.car.tesla.preap.tools.safety import require_offroad
    require_offroad()
  else:
    if not confirmed:
      p("")
      p("RISK ACKNOWLEDGMENT REQUIRED")
      p("Re-run with --accept-risk or start from UI and confirm.")
      return 1
    require_preap_tool_start(tool=tool_name, confirmed=True)

  transport = None
  try:
    if not args.extract_only:
      # Integrity MUST pass before panda USB is opened.
      p("\nVerifying bootloader integrity before connect...")
      verify_bootloader(bl_path)
      p("  Bootloader size/SHA-256/MD5 OK")
    ...
    flash_bootloader(uds_client, bl_path, BOOTLOADER_ADDR)
    ...
    return 1
  except (BootloaderIntegrityError, TransportError) as e:
    p(f"\nERROR: {e}")
    return 1
  except NegativeResponseError as e:
    try:
      fail_closed_negative_response(e)
